...
//         </div>
//       )}
// {/* ✅ New Triage Request — show when completed */}
// {triageData.status === "COMPLETED" && (
//   <Card className="border-green-200 bg-green-50">
//     <CardContent className="pt-6 text-center">
//       <CheckCircle className="h-12 w-12 text-green-600 mx-auto mb-3" />
//       <h3 className="font-semibold text-green-900 text-lg mb-1">
//         Your triage case is complete
//       </h3>
//       <p className="text-sm text-green-700 mb-4">
//         Need to submit new symptoms? Start a fresh triage request.
//       </p>
//       <Button 
//         className="bg-green-600 hover:bg-green-700"
//         onClick={() => navigate("/dashboard/patient/symptoms")}
//       >
//         Start New Triage Request
//         <ArrowRight className="ml-2 h-4 w-4" />
//       </Button>
//     </CardContent>
//   </Card>
...
// ---------- Helpers (component ke bahar — ek hi baar bante hain) ----------
...
const TriageStatus = () => {
  ...
  useEffect(() => {
    const fetchTriage = async () => {
      // pehli baar full loader, baad mein sirf subtle refreshing
      if (triageData) setRefreshing(true);
      ...
      try {
        ...
        const active = sorted.find((c) => ACTIVE_STATUSES.includes(c.status));
        setTriageData(active ?? sorted[0] ?? null);
      } catch (err) {
        console.error("Failed to load triage:", err);
        setError(err.message || "Failed to load triage status");
      } finally {
        setLoading(false);
        setRefreshing(false);
      }
      ...
      <div className="space-y-6">
        ...
        <Card>
          <CardContent className="pt-12 pb-12 text-center">
            ...
            <p className="text-gray-500 mt-2">
              You don't have any pending triage cases. If you're experiencing
              symptoms, please submit them for review.
            </p>
            <Button
              className="mt-4 bg-green-600 hover:bg-green-700"
              onClick={() => navigate("/dashboard/patient/symptoms")}
  ...
  const timelineSteps = [
    ...
    },
    {
      ...
      time:
        triageData.status !== "PENDING"
          ? formatDate(triageData.updatedAt)
          : "Waiting...",
    },
    {
      status: "Provider Assignment",
  ...
  return (
    <div className="space-y-6 relative">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            {triageData.assignedProvider && (
              <div>
                <p className="text-sm text-gray-500">Provider Assigned</p>
                <p className="font-medium">
                  Dr. {triageData.assignedProvider.firstName}{" "}
                  {triageData.assignedProvider.lastName}
                </p>
                <p className="text-sm text-gray-500">
                  {triageData.assignedProvider.specialty}
                </p>
      ...
      {/* ===== COMPLETED — single consolidated card ===== */}
      {triageData.status === "COMPLETED" && (
        <Card className="border-green-200 bg-green-50">
          <CardContent className="pt-6 text-center">
            <CheckCircle className="h-12 w-12 text-green-600 mx-auto mb-3" />
          ...
          <CardContent className="pt-6">
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 flex items-start gap-3">
              ...
              <div>
                <p className="font-medium text-yellow-800">Waiting for review</p>
                <p className="text-sm text-yellow-700">
                  A triage nurse will review your symptoms shortly. You'll
                  receive a notification when there's an update.
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="pt-6">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start gap-3">
              ...
              <div>
                <p className="font-medium text-blue-800">Under Review</p>
                <p className="text-sm text-blue-700">
                  A triage nurse is currently reviewing your symptoms. You'll
                  be notified once a provider is assigned.
                </p>
              </div>
            </div>
          </CardContent>
