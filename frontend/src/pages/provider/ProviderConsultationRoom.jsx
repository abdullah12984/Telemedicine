...
import { useNavigate, useParams } from "react-router-dom";
import { socket } from "../../services/socket";
import { getCameraStream } from "../../services/mediaStreamHelper"; // ✅ real camera
import { Button } from "@/components/ui/button";
import { Badge } from "@/components/ui/badge";
import {
  Video, VideoOff, Mic, MicOff, PhoneOff, ScreenShare,
  User, Clock, MessageSquare, Loader2, Maximize2, Minimize2,
  AlertCircle,
} from "lucide-react";
import {
...
const ProviderConsultationRoom = () => {
  ...
  const videoRef = useRef(null);            // 🟢 BADA — Doctor's OWN camera
  const localVideoRef = useRef(null);       // 🔴 CHOTA — Patient's camera
  const timerRef = useRef(null);
  const peerConnectionRef = useRef(null);
  const localStreamRef = useRef(null);
  const screenStreamRef = useRef(null);
  ...
  const rtcConfiguration = {
    iceServers: [
      { urls: "stun:stun.l.google.com:19302" },
      { urls: "stun:stun1.l.google.com:19302" },
      // 🔴 Production ke liye TURN zaroor add karo
  ...
  // ✅ SWAPPED: Attach DOCTOR'S OWN stream to the BIG video
  useEffect(() => {
    if (!loading && videoRef.current && localStreamRef.current) {
      console.log("📺 Attaching LOCAL (doctor) stream to BIG screen");
      videoRef.current.srcObject = localStreamRef.current;
      videoRef.current.play().catch((err) => console.log("Play error:", err));
    }
  }, [loading]);

  // ✅ SWAPPED: Attach PATIENT'S stream to the SMALL PiP
  useEffect(() => {
    if (localVideoRef.current && remoteStream) {
      console.log("📺 Attaching REMOTE (patient) stream to PiP");
  ...
  useEffect(() => {
    ...
    const setupCall = async () => {
      try {
        ...
        pc.onicecandidate = (event) => {
          if (event.candidate) {
            socket.emit("ice-candidate", { consultationId, candidate: event.candidate });
          }
        };
        ...
        console.error("Doctor setup error:", err);
        setErrorMessage(err.message || "Could not start the consultation.");
        setLoading(false);
      }
    };
  ...
  return (
    <div className="h-[calc(100vh-120px)] flex flex-col">
      ...
      <div className="flex items-center justify-between p-3 bg-gray-900 text-white rounded-t-lg">
        <div className="flex items-center gap-3">
          <div className={`w-3 h-3 rounded-full ${
            ...
            : isPatientJoined ? "bg-yellow-500 animate-pulse"
            : "bg-red-500"
          }`} />
          <span className="text-sm font-medium">
            {isConnected ? "Connected with Patient"
             : isPatientJoined ? "Calling Patient..."
             : "Waiting for patient..."}
          </span>
          <Badge className="bg-white/20 text-white border-0">
            <Clock className="h-3 w-3 mr-1" />
      ...
      {/* ============================================
          VIDEO GRID (SWAPPED LAYOUT)
          BIG   = Doctor's own camera (videoRef)
          SMALL = Patient's camera (localVideoRef)
      ============================================ */}
      <div className="flex-1 bg-gray-900 relative">
        <div className="w-full h-full flex items-center justify-center">

          {/* 🟢 BIG: Doctor's OWN camera — ALWAYS visible */}
          <video
            ref={videoRef}
          ...
          {!isConnected && (
            <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/60 text-white text-center p-4 pointer-events-none">
              <User className="h-20 w-20 mb-3 opacity-50" />
              <p className="text-xl font-semibold">
                {isPatientJoined ? "Connecting to Patient..." : "Waiting for Patient"}
              </p>
        ...
        <div className="absolute top-4 right-4 w-52 h-36 bg-gray-800 rounded-lg border-2 border-white/30 overflow-hidden shadow-lg z-10">
          ...
          <div className="absolute bottom-1 left-2 text-xs text-white/90 bg-black/60 px-1.5 py-0.5 rounded">
            {isConnected ? "Patient" : "Waiting for Patient"}
          </div>
        </div>
